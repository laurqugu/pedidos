...
import com.mycompany.dbpedidos.service.Hello;
...
public class PedidosFacadeREST extends AbstractFacade<Pedidos> {
    ...
    public void create(Pedido pedido) {
        ...
        ped.setTipodepedido(pedido.getTipoPedido());
        ped.setCodproveedor(pedido.getCodProveedor());
        ped.setCodpuntodeentraga(pedido.getCodPuntoEntrega());
        ped.setCodestadobodega(pedido.getEstadoBodega());
        ped.setCodestadopedido(pedido.getEstadoPedido());
        ped.setCodestadoproveedor(pedido.getEstadoProveedor());
        super.create(ped);        
        for(Prodpedido p:pedido.getProductos()){
            Productopedido pp = new Productopedido();
            pp.setCodpedido(ped.getCodpedido());
            pp.setCodproveedor(pedido.getCodProveedor());
            pp.setCodproducto(p.getPLU());
            pp.setCantidadproducto(p.getCantidadProducto());  
            pp.setCantidadconfirmada(0);
            em.persist(pp);
        }
